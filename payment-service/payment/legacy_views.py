    ...
    def post(self, request):
        try:
            payment = _pay_svc.process_payment(
                ...
                amount=float(request.data.get("payment_amount", request.data.get("amount", 0))),
                method_id=int(request.data.get("payment_method_id", 0)) or None,
            )
...
class InternalPaymentView(APIView):
    ...
    def post(self, request):
        try:
            payment = _pay_svc.process_payment(
                order_id=int(request.data["order_id"]),
                amount=float(request.data.get("amount", request.data.get("payment_amount", 0))),
                method_id=int(request.data.get("payment_method_id", 0)) or None,
            )
            return Response(PaymentSerializer(payment).data, status=status.HTTP_201_CREATED)
        except (KeyError, ValueError) as e:
